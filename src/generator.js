...
export default function generate(program) {
  ...
  const generators = {
    ...
    BooleanComparison(node) {
      let compareOp =
        node.comparisonOperator != "==" ? node.comparisonOperator : "===";
    ...
    FunctionCallExpression(node) {
      output.push(
        `${node.functionEntity.name}(${node.argumentsExpression.argumentValues.map((e) => gen(e.expression)).join(", ")})`,
      );
    },
    ...
    FunctionDeclaration(node) {
      ...
      output.push(
        `function ${functionEntity.name}(${functionEntity.params.parameters.map((e) => e.id).join(", ")}) {`,
      );
    ...
    VariableOperatorAssignment(node) {
      output.push(
        `${node.variable.name} ${node.incrementOperator} ${gen(node.value)}`,
      );
    },
    ArrayAssignment(node) {
      output.push(
        `${node.variable.name}[${gen(node.index)}] = ${gen(node.value)}`,
      );
    },
    PrintFunction(node) {
    ...
    ForLoop(node) {
      ...
      let conditionStr = gen(node.condition);
      let iteratorStr = node.iterator.pre
        ? `${node.iterator.iterationOperator}${node.iterator.variable.name}`
        : `${node.iterator.variable.name}${node.iterator.iterationOperator}`;
