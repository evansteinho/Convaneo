...
export default function analyze(match) {
  ...
  function typeCastable(type1, type2) {
    ...
    return (
      (numericTypes.includes(type1) && numericTypes.includes(type2)) ||
      type1 === type2 ||
  ...
  function numericTypeCondition(entity, location) {
    const numericTypes = [core.intType, core.floatType, core.doubleType];
    checkCondition(
      numericTypes.includes(entity.type),
  ...
  function similarTypeCondition(entity1, entity2, location) {
    checkCondition(
      assignable(entity1.type, entity2.type),
      "Operands do not have similar types",
      location,
    );
  }
  function assignableIntoArrayCondition(arrayEntity, otherEntity, location) {
    checkCondition(
      assignable(arrayEntity.type.elementType, otherEntity.type),
      `Array containing ${arrayEntity.type.elementType} type cannot contain ${otherEntity.type} type`,
      location,
    );
  }
  ...
  function typeCastableCondition(type1, type2, location) {
    checkCondition(
      typeCastable(type1, type2),
      `Cannot cast type ${type1} to ${type2}`,
      location,
    );
  }
  ...
  function inFunctionCondition(location) {
    checkCondition(
      context.funcEntity !== null,
      "Return can only appear in  function",
      location,
    );
  }
  function voidReturnCondition(functionEntity, location) {
    checkCondition(
      functionEntity.type.returnType === core.voidType,
      `Non-void function attempted to return void type`,
      location,
    );
  }
  function anyReturnCondition(functionEntity, location) {
    checkCondition(
      functionEntity.type.returnType !== core.voidType,
      `Void function attempted to return a non-void type`,
      location,
    );
  }
  function returnableCondition(entity, functionEntity, location) {
    checkCondition(
      equivalent(entity.type, functionEntity.type.returnType),
      `A function with a return type of ${functionEntity.type.returnType} cannot return a ${entity.type}`,
      location,
    );
  }
  function indexMustBeIntCondition(index, location) {
    checkCondition(
      index.type === core.intType,
      `Index for array must be have an int value`,
      location,
    );
  }
  ...
  const builder = match.matcher.grammar.createSemantics().addOperation("rep", {
    ...
    FunctionDeclaration(type, id, _open, parameters, _close, block) {
      ...
      const paramTypes = params.parameters.map((parameterTerm) =>
        parameterTerm.type.rep(),
      );
      const functionType = core.functionType(paramTypes, type.rep());
      const funcEntity = core.functionEntity(
        id.sourceString,
        params,
        null,
        functionType,
    ...
    VariableDeclaration(type, id, _assignmentKey, expression, _semicolon) {
      ...
      if (t?.kind === "ArrayType") {
        ...
        const arrayEntity = core.arrayEntity(
          id.sourceString,
          t,
          expression.size,
        );
    ...
    IfBlock(ifStatement, elseIfStatements, elseStatement) {
      ...
      if (elseIfStatements.children.length != 0) {
        elseifEntities = elseIfStatements.children.map((elseIfStatement) =>
          elseIfStatement.rep(),
        );
      }
    ...
    FunctionCallExpression(id, _open, argumentsExpression, _close) {
      ...
      const argTypes = args.argumentValues.map(
        (argValExpression) => argValExpression.expression.type,
      );
      ...
      return core.functionCallExpression(
        functionEntity,
        args,
        functionReturnType,
      );
    ...
    Parameters(parameterTerms) {
      const parameters = parameterTerms.children.map((parameterTerm) =>
        parameterTerm.rep(),
      );
    ...
    ArgumentsExpression(argumentValues) {
      const argumentsVals = argumentValues.children.map((argumentValue) =>
        argumentValue.rep(),
      );
    ...
    Typecast_functionCall(_open, type, _close, functionCallExpression) {
      ...
      return core.typecastFunctionCall(
        typeToCastTo,
        core.functionCallValue(
          functionCall.functionEntity.type.returnType,
          functionCall,
        ),
      );
    },
    ...
    NumericPrimary_iterPost(id, iterationOperator) {
      ...
      const iterOp = iterationOperator.sourceString;
      return core.numericValue(
        variableEntity.type,
        core.iterPostVariable(variableEntity.type, variableEntity, iterOp),
      );
    },
    NumericPrimary_iterPre(iterationOperator, id) {
      ...
      return core.numericValue(
        variableEntity.type,
        core.iterPreVariable(variableEntity.type, variableEntity, iterOp),
      );
    },
    ...
    NumericPrimary_funcs(functionCallExpression) {
      ...
      return core.functionCallValue(
        functionCallExp.functionEntity.type.returnType,
        functionCallExp,
      );
    },
    ...
    BooleanExpression_mult(boolExp, optionalLogicOp, optionalBoolExp) {
      ...
      let mainBoolExp = core.booleanExpression(
        core.boolType,
        mainExp,
        logicOps[0],
        boolExps[0],
