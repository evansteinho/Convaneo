...
const optimizers = {
  ...
  NumericExpression(e) {
    ...
    if (e.left.kind === "NumericValue" && e.right.kind === "NumericValue") {
      ...
      return core.numericValue(
        e.left.type,
        eval(`${e.left.value} ${e.op} ${e.right.value}`),
      );
    }
    return e;
  },
  ...
  BooleanExpression(e) {
    ...
    if (e.left.kind === "BooleanValue" && e.right.kind === "BooleanValue") {
      return core.booleanValue(
        core.boolType,
        eval(`${e.left.value} ${e.logicOperator} ${e.right.value}`),
      );
    }
    return e;
  },
  BooleanComparison(e) {
    ...
    if (e.left.sourceString && e.right.sourceString) {
      return core.booleanValue(
        core.boolType,
        eval(`${e.left.sourceString} ${compOp} ${e.right.sourceString}`),
      );
    }
    ...
    if (e.left.kind === "NumericValue" && e.right.kind === "NumericValue") {
      return core.booleanValue(
        core.boolType,
        eval(`${e.left.value} ${compOp} ${e.right.value}`),
      );
    }
